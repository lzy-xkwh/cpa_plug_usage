...
插件 v0.7.0 起向导完全傻瓜式：**已配置的供应商自动显示余额，只有自动搞
不定的才需要补配置**。

入口：管理后台 **插件 → api-balance → 「余额配置向导」**（或直接访问
`/v0/resource/plugins/api-balance/config-wizard`）。

- 页面自动列出 CPA 里已配置的全部供应商，并标注状态：
  - ✅ **已支持**：余额已在供应商页签显示，无需任何配置；
  - ❌ **无法查询**：OpenAI / Claude / Gemini 等官方没有公开余额接口；
  - ⚠️ **可配置**：勾选 → 选厂商类型（字段自动带出）→ 保存。
- 保存通过服务端直调 CPA 管理 API 完成，配置立即热加载；
- 首次使用粘贴一次 CPA 管理密钥即可（保存在插件配置 `management_key`，
  不会下发到页面；向导自动读取供应商列表、保存配置都靠它，之后不再询问）；
  **填错了随时点「重设管理密钥」重填**——密钥被 CPA 拒绝（401/403）时
  向导也会自动展开重填入口；也可在插件配置 YAML 中直接写 `management_key`；
- 可选 `management_url`：CPA 服务地址，默认 `http://127.0.0.1:8317`；

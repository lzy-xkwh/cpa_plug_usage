...
插件 v0.6.0 起内置管理后台入口：**插件 → api-balance → 「余额配置向导」**
菜单（或直接访问
`/v0/resource/plugins/api-balance/config-wizard`）。

向导提供中文表单：

- 选厂商（DeepSeek / Kimi / OpenRouter / one-api 系 / New API / sub2api /
  自定义），默认字段自动带出；
- 一个档案一个凭据来源，多个厂商点「＋ 添加厂商档案」；
- 可选填 CPA 管理密钥后，直接在本页「读取现有配置 / 保存到 CPA」；
- 不填密钥也能一键复制生成的 YAML，粘贴到插件配置里保存即可。
